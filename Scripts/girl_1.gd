...
# movement and interaction tuning — editable from the inspector
...
func _ready() -> void:
	...
	# disable collision for a couple seconds so she doesn't block the doorway coming in
...
func _physics_process(_delta: float) -> void:
	# walking out — move forward until she's off screen then clean up
	if _leaving:
	...
	# reached the standing position, stop and wait a beat before allowing interaction
...
func _process(delta: float) -> void:
	...
	# offset the check point because her model isn't centred on the node
...
func _start_leaving() -> void:
	# play the leave animation and walk her out
	interaction_label.visible = false
	_leaving = true
